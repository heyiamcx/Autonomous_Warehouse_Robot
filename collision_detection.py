...
def find_nearest_charging_station(pos):
    return min(charging_stations, key=lambda cs: manhattan_distance(pos, cs))
...
while running:
    ...
    for robot in pink_robots:
        ...
        # Find a shelf to restock if needed
        ...
        # Moving to pickup point to get a package
        if robot['step'] == 'to_pickup':
            if robot['pos'] == list(robot['pickup_point']):
                if pickup_packages[tuple(robot['pickup_point'])] > 0:
                    ...
                    # Calculate path to target shelf
        ...
        # Moving to shelf to place a package
    ...
    if multi_red_cooldown <= 0:
        eligible = [s for s in shelves if shelf_inventory[s] == 'full' and s not in assigned_red_shelves]
    ...
    for robot in green_robots:
        ...
        # Moving to shelf to pick up a package
        if robot['task'] == 'pickup':
            if robot['pos'] == list(robot['target_shelf']):
                ...
                # Add a package to in_transit
                in_transit_packages.append({'pos': list(robot['pos']), 'carried_by': robot})
                
                # Choose a delivery point and calculate path
        ...
        # Moving to delivery point to drop off package
